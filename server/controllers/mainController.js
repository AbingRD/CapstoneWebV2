...
const authGoogleCallBack = asyncHandler(async (req, res) => {
    ...
    res.redirect(
        process.env.node_env == "dev"
            ...
            : `https://road-ready-black.vercel.app/dashboard?api_access_token=${token}&access_token=${data.session.access_token}`
    );
    return;
});
...
const login = asyncHandler(async (req, res) => {
    try {
        ...
        return res
            .status(200)
            .json({ status: true, message: "Login success", data: { user } });
    } catch (error) {
        console.log(error);
        return res.status(500).json({ status: false, message: error.message });
    }
...
const requestOTPCode = asyncHandler(async (req, res) => {
    try {
        ...
        if (user.firstname.trim().length == 0 || user.lastname.trim().length == 0 || user.phonenumber.trim().length == 0 || user.address.trim().length == 0) {
...
const createListing = asyncHandler(async (req, res) => {
    try {
        ...
        const imageURL =
            process.env.supabaseURL + `/storage/v1/object/public/` + imageData.fullPath;
        query =
...
const getDealershipBankAgents = asyncHandler(async (req, res) => {
    try {
        ...
        const bankagents = (await pool.query(query, [req.tokenData.dealership.id])).rows;
        return res.status(200).json({ status: true, message: "Successfully fetched bank agents", data: { bankagents } })
    } catch (error) {
        console.error(error);
        return res.status(500).json({ status: false, message: error.message });
    }
    ...
    try {
        ...
        switch (applicationType) {
            case "cash":
                ...
                if (progress == -1) {
                    query =
                        `UPDATE ${table} SET agent = $1, status = 'rejected', updatedAt = NOW() WHERE id = $2 RETURNING *;`
                    application = (
                        await pool.query(query, [req.tokenData.id, applicationId])
                    ).rows[0];
                } else if (progress == 1) {
                    query =
                        `UPDATE ${table} SET progress = $1, agent = $2, status = 'on-going', updatedAt = NOW() WHERE id = $3 RETURNING *;`
                    application = (
                        await pool.query(query, [progress, req.tokenData.id, applicationId])
                    ).rows[0];
                } else if (progress == 4) {
                    query =
                        `UPDATE ${table} SET progress = $1, status = 'completed', updatedAt = NOW() WHERE id = $2 RETURNING *;`
                    application = (await pool.query(query, [progress, applicationId])).rows[0]
                    ...
                    const applicationPDFUploadURL =
                        `https://xjrhebmomygxcafbvlye.supabase.co/storage/v1/object/public/` +
                        applicationPDFUpload.data.fullPath;

                    query =
                        `UPDATE ${table} SET applicationpdf = $1 WHERE id = $2 RETURNING *;`
                    application = (await pool.query(query, [applicationPDFUploadURL, applicationId]))
                        .rows[0];
                    ...
                    query =
                        `UPDATE ${table} SET progress = $1, updatedAt = NOW() WHERE id = $2 AND agent = $3 RETURNING *;`
                    application = (
            ...
            case "bankLoan(dealershipBankChoice)":
                ...
                if (progress < 4 || progress == 7) {
                    ...
                    if (progress == 3) {
                        ...
                        if (fieldsValidation) return fieldsValidation
                        query =
                            `UPDATE ${table} SET progress = $1, bankAgent = $2, updatedAt = NOW() WHERE id = $3 and agent = $4 RETURNING *;`
                        application = (
                            await pool.query(query, [progress, req.body.bankAgentId, applicationId, req.tokenData.id])
                        ).rows[0];
                    }
                    if (progress == 7) {
                        query =
                    ...
                    if (progress == 6) {
                        const certificateUpload = await uploadFile(
                            req.file,
                            "certificate"
                        );
                        ...
                        query =
                            `UPDATE ${table} SET progress = $1, loancertificate = $2, updatedAt = NOW() WHERE id = $3 and bankAgent = $4 RETURNING *;`
                        application = (
                            await pool.query(query, [progress, certificateUploadURL, applicationId, req.tokenData.id])
                        ).rows[0];
                    } else {
                        query =
                            `UPDATE ${table} SET progress = $1, updatedAt = NOW() WHERE id = $2 and bankAgent = $3 RETURNING *;`
                        application = (
                            await pool.query(query, [progress, applicationId, req.tokenData.id])
                        ).rows[0];
                    }
                ...
                switch (parseInt(progress)) {
                    ...
                    case 6:
                        ...
                        break;
                    case 7:
                        query = "INSERT INTO tblNotifications (userId, notification) VALUES ($1, 'Your application for listing ' || $2 || ' is completed')"
                        await pool.query(query, [application.buyer, application.listing]);
                        break;
                }
        ...
        return res.status(401).json({ status: false, message: "Invalid application type" });
    } catch (error) {
        console.error(error);
        return res.status(500).json({ status: false, message: error.message });
    }
...
const getBankAgentApplicants = asyncHandler(async (req, res) => {
    try {
        let query = "SELECT * FROM tblDealershipBankLoanApplication WHERE bankagent = $1";
        const applications = (await pool.query(query, [req.tokenData.id])).rows
        return res.status(200).json({ status: false, message: "Successfully fetched applications", data: { applications } })
    } catch (error) {
        console.error(error);
        return res.status(500).json({ status: false, message: error.message });
    }
...
const buyerCreateApplication = asyncHandler(async (req, res) => {
    try {
        ...
        let fieldsValidation = validateRequiredFields(
            requiredFields,
            req.body,
            res
        );
        ...
        if (applications.length > 1) return res.status(400).json({ status: false, message: "You have reached the limit for the number of applications." });
        ...
        const signatureURL =
            process.env.supabaseURL + `/storage/v1/object/public/` + signatureUpload.fullPath;
        const validIdURL =
            process.env.supabaseURL + `/storage/v1/object/public/` +
        ...
        switch (modeofpayment) {
            case "cash":
                ...
                fieldsValidation = validateRequiredFields(
                    requiredFields,
                    req.body,
                    res
                );
            ...
            case "inhouseFinance":
                ...
                const coMakerSignatureURL =
                    process.env.supabaseURL + `/storage/v1/object/public/` + coMakerSignatureUpload.fullPath;
                const coMakerValidIdURL =
                    process.env.supabaseURL + `/storage/v1/object/public/` + coMakerValidIdUpload.fullPath;
            ...
            case "bankLoan(buyerBankChoice)":
                ...
                const bankCertificateURL =
                    process.env.supabaseURL + `/storage/v1/object/public/` + bankCertificateUpload.fullPath;
        ...
        const assetlinksData = [
            {
                ...
                target: {
                    ...
                    package_name: "com.example.roadreadyv3_test",
...
const generatePDF = async (application) => {
    ...
    try {
        const html = await htmlPDF.readFile(path.resolve(__dirname, "../views/application.handlebars"), 'utf-8');
