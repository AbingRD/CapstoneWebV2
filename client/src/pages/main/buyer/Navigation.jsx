    ...
    return (
        <div className='h-[12vh] border border-b-[#CBD5E1] border-t-[#9CA3AF] border-x-[#9CA3AF] bg-[#eeeeee] flex flex-row items-center px-10 justify-between fixed top-0 left-0 w-screen shadow-md z-40'>
            ...
            <div className='flex flex-row gap-4'>
                ...
                <button className='border border-black px-2 py-1 rounded-lg flex flex-row gap-2 items-center shadow-md hover:text-[#4f41bc] transition-all' onClick={() => setActiveNav("applications")}>
                    <p>Application Progress</p>
                    <FaWallet className={`text-3xl ${activeNav == 'applications' ? 'text-[#4f41bc]' : ''}`} />
                </button>

                <button className='border border-black px-2 py-1 rounded-lg flex flex-row gap-2 items-center shadow-md hover:text-[#4f41bc] transition-all' onClick={() => setActiveNav("myDocuments")}>
                    <p>My Documents</p>
                    <FaCar className={`text-3xl ${activeNav == 'myDocuments' ? 'text-[#4f41bc]' : ''}`} />
                </button>

                <button className='border border-black px-2 py-1 rounded-lg flex flex-row gap-2 items-center shadow-md hover:text-[#4f41bc] transition-all' onClick={() => setActiveNav("notifications")}>
                    <p>Notifications</p>
