...
function BankLoanBuyerBankChoiceApplicationProgress({ application, handleRejectApplication, handleUpdateApplication, confirmRejectApplicationModalRef }) {
            ...
            <table className='border border-black'>
                <tbody>
                    <tr className={`p-2 border border-b-black flex ${application.progress >= 1 && 'bg-[#34F481]'}`}>
                        <td className='flex flex-row gap-2 items-center'>
                            ...
                            {application.progress == 0 && (
                                <>
                                    <button onClick={() => handleUpdateApplication(1, "bankLoan(buyerBankChoice)")} className='px-2 py-1 rounded-lg border border-black'>Accept</button>
                                        ...
                                        {(application.status == "on-going" && userData.id == application.agent) && (
                                            <>
                                                <div className='flex flex-row gap-2'>
                                                    <button onClick={() => handleUpdateApplication(2, "bankLoan(buyerBankChoice)")} className='px-2 py-1 rounded-lg border border-black'>Accept</button>
                                                    <button onClick={() => openModal(confirmRejectApplicationModalRef)} className={`px-2 py-1 rounded-lg border border-black`}>Reject</button>
                                                </div>
                                            </>
                                        )}
                    ...
                    <tr className={`p-2 border border-b-black flex flex-row gap-2 items-center ${application.progress >= 3 && 'bg-[#34F481]'} ${(application.status == 'rejected' && application.progress < 3) && 'bg-red-500'}`}>
                        <td>
                            <div className='flex flex-row gap-2 items-center'>
                                <p>3. Buyer Bank Loan Certificate.</p>
                                ...
                                {application.progress == 2 && (
                                    < div className='flex flex-row gap-2 items-center'>
                                        {(application.status == "rejected" && application.progress == '2') && (
                                            <p className='text-white'>Buyer Bank Loan Certificate Rejected</p>
                                        )}
                                        {(application.status == "on-going" && userData.id == application.agent) && (
                                            <>
                                                <div className='flex flex-row gap-2'>
                                                    <button onClick={() => handleUpdateApplication(3, "bankLoan(buyerBankChoice)")} className='px-2 py-1 rounded-lg border border-black'>Accept</button>
                                                    <button onClick={() => openModal(confirmRejectApplicationModalRef)} className={`px-2 py-1 rounded-lg border border-black`}>Reject</button>
                                                </div>
                                            </>
                                        )}
                                    ...
                                    <>
                                        {
                                            (application.status == "on-going" && userData.id == application.agent) && (
                                                <>
                                                    <div className='flex flex-row gap-2'>
                                                        <button onClick={() => handleUpdateApplication(4, "bankLoan(buyerBankChoice)")} className='px-2 py-1 rounded-lg border border-black'>Release</button>
                                                    </div>
                                                </>
                                            )
                                        }
            ...
            <dialog ref={confirmRejectApplicationModalRef} className='bg-gray-100 rounded-lg'>
                ...
                <div className='flex flex-col gap-4 justify-center border p-10 text-sm'>
                    ...
                    <div className='flex flex-row gap-4 font-semibold w-full justify-center items-center'>
                        <button onClick={() => handleRejectApplication("bankLoan(buyerBankChoice)")} className='border border-black rounded-lg px-2 py-1'>Yes</button>
...
export default BankLoanBuyerBankChoiceApplicationProgress