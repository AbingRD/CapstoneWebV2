...
function InhouseFinanceApplicationProgress({ application, handleRejectApplication, handleUpdateApplication, confirmRejectApplicationModalRef }) {
        ...
        <div className='w-full flex flex-row gap-10 justify-center'>
            <div className='flex flex-row gap-5'>
                <div className=''>
                    ...
                    <p>Phone Number: </p>
                    <p>Co Maker Name: </p>
                    <p>Co Maker Address: </p>
                    <p>Co Maker Phone Number: </p>
                </div>
                <div className='text-end font-bold text-md '>
                    <p>{application.firstname} {application.lastname}</p>
                    <p>{application.address}</p>
                    <p>{application.phonenumber}</p>
                    <p>{application.comakerfirstname} {application.comakerlastname}</p>
                    <p>{application.address}</p>
                    <p>{application.phonenumber}</p>
                </div>
            </div>
            <table className='border border-black'>
                <tbody>
                    <tr className={`p-2 border border-b-black flex ${application.progress >= 1 && 'bg-[#34F481]'}`}>
                        <td className='flex flex-row gap-2 items-center'>
                            ...
                            {application.progress == 0 && (
                                <>
                                    <button onClick={() => handleUpdateApplication(1, "inhouseFinance")} className='px-2 py-1 rounded-lg border border-black'>Accept</button>
                                        ...
                                        {(application.status == "on-going" && userData.id == application.agent) && (
                                            <>
                                                <div className='flex flex-row gap-2'>
                                                    <button onClick={() => handleUpdateApplication(2, "inhouseFinance")} className='px-2 py-1 rounded-lg border border-black'>Accept</button>
                                                    <button onClick={() => openModal(confirmRejectApplicationModalRef)} className={`px-2 py-1 rounded-lg border border-black`}>Reject</button>
                                                </div>
                                            </>
                                        )}
                    ...
                    <tr className={`p-2 border border-b-black flex flex-row gap-2 items-center ${application.progress >= 3 && 'bg-[#34F481]'} ${(application.status == 'rejected' && application.progress < 3) && 'bg-red-500'}`}>
                        <td>
                            <div className='flex flex-row gap-2 items-center'>
                                <p>3. Credit Investigation.</p>

                                {application.progress == 2 && (
                                    < div className='flex flex-row gap-2 items-center'>
                                        {(application.status == "rejected" && application.progress == '2') && (
                                            <p className='text-white'>Credit Investigation Rejected</p>
                                        )}
                                        {(application.status == "on-going" && userData.id == application.agent) && (
                                            <>
                                                <div className='flex flex-row gap-2'>
                                                    <button onClick={() => handleUpdateApplication(3, "inhouseFinance")} className='px-2 py-1 rounded-lg border border-black'>Accept</button>
                                                    <button onClick={() => openModal(confirmRejectApplicationModalRef)} className={`px-2 py-1 rounded-lg border border-black`}>Reject</button>
                                                </div>
                                            </>
                                        )}
                                    ...
                                    <>
                                        {
                                            (application.status == "on-going" && userData.id == application.agent) && (
                                                <>
                                                    <div className='flex flex-row gap-2'>
                                                        <button onClick={() => handleUpdateApplication(4, "inhouseFinance")} className='px-2 py-1 rounded-lg border border-black'>Release</button>
                                                    </div>
                                                </>
                                            )
                                        }
            ...
            <dialog ref={confirmRejectApplicationModalRef} className='bg-gray-100 rounded-lg'>
                ...
                <div className='flex flex-col gap-4 justify-center border p-10 text-sm'>
                    ...
                    <div className='flex flex-row gap-4 font-semibold w-full justify-center items-center'>
                        <button onClick={() => handleRejectApplication("inhouseFinance")} className='border border-black rounded-lg px-2 py-1'>Yes</button>
            ...
            <dialog ref={buyerDocumentsModalRef} onClick={(e) => handleDialogOutsideClick(e, buyerDocumentsModalRef)} className='bg-gray-100 rounded-lg'>
                <div className='flex flex-row gap-3'>
                    ...
                    <div className='flex flex-col gap-4 justify-center border p-10 text-sm font-semibold'>
                        <div className='flex flex-row gap-2'>
                            <div className='flex flex-col gap-2 w-full items-center'>
                                <p>Co Maker Signature</p>
                                <div className='w-[200px]'>
                                    <img src={application.comakersignature} alt="Co Maker Signature" />
                                </div>
                            </div>
                            <div className='flex flex-col gap-2 w-full items-center'>
                                <p>Co Maker Buyer Valid ID</p>
                                <div className='w-[200px]'>
                                    <img src={application.comakervalidid} alt="Co Maker Valid ID" />
                                </div>
                            </div>
                        </div>
                    </div>
...
export default InhouseFinanceApplicationProgress